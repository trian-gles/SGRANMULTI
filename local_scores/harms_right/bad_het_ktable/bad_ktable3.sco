...
for (i = 0; i < harms; i = i + 1){
    ...
    amp_duty = irand(0.1, 0.3)
	lag = irand(0.65, 0.8)
	// (float cycles, float phase, float duty, float lag, float numpoints)
	amp = make_ktable(amp_cycles, phase, amp_duty, lag, 1000)
    ...
    SGRAN2MULTI(inskip, dur, 2000 * amp * mast_amp, ratelo, ratemid, ratehi, rateti, durlo, durmid, durhi, durti, 
