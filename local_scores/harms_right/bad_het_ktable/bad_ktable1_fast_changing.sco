...
include ../table_gen/tablegenbetter.sco
...
for (h = 0; h < 50; h = h + 1)
{
	dist_coef = irand(0.95, 1.05)
	shift = irand(-50, 50)

	for (i = 0; i < harms; i = i + 1){

		
		ratelo = irand(0.0009, 0.0048)
		...
		durlo = ratelo * irand(1, 3) // maketable("line", "nonorm", 200, 0, log(trand(2, 10)) / trand(20, 90), 1, log(trand(2, 10)) / trand(20, 90))
		...
		mult = i + 1
		phase = irand(0, 1)
		amp_cycles = irand(3, 6)
		amp_duty = irand(0.1, 0.2)
		lag = 0.4
		// (float cycles, float phase, float duty, float lag, float numpoints)
		amp = make_ktable(amp_cycles, phase, amp_duty, lag, 4000)
		...
		SGRAN2MULTI(st, dur, 400 * amp * mast_amp, ratelo, ratemid, ratehi, rateti, durlo, durmid, durhi, durti, 
