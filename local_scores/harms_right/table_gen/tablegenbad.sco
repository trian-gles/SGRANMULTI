...
handle make_ktable(float cycles, float phase, float duty, float lag, float numpoints){
    ...
    base_table = maketable("linebrk", "nonorm", single_cycle_points, 1, (duty * single_cycle_points), 1, 0, 0, ((1-duty) * single_cycle_points + 1))
    ...
    if (remainder > 0){
        remain_table = maketable("linebrk", "nonorm", remainder, 0, remainder, 0, 0)
