...
freqlo = 400 / 4// maketable("line", "nonorm", 200, 0, 400, 1, 200)
freqmid = 430 / 4//maketable("line", "nonorm", 200, 0, 430, 1, 350, 2, 600)
freqhi = 440 / 4//maketable("line", "nonorm", 200, 0, 440, 1, 460, 2, 800)
...
for (i = 0; i < harms; i = i + 1){
	...
	phase = rand() * 360
	rate_cycles = rand() + 4
	ratebase = (maketable("wave3", 1000, rate_cycles, 1, phase) + 1)
	ratelo = (maketable("wave3", 1000, rate_cycles, 1, phase) + 1)
	steepness = 2

	for (j = 0; j < steepness - 1; j = j + 1){
		ratelo = mul(ratelo, ratebase)
	}
	ratelo = ratelo / pow(2, steepness)

	ratelo = ((ratelo * -1) + 1.0004) / 16
	ratemid = ratelo * 10
	ratehi = ratelo * 20
	rateti = 1// maketable("line", "nonorm", 200, 0, 8, 1, 0.2)

	durlo = randrange(0.001, 0.1) // maketable("line", "nonorm", 200, 0, randrange(0.001, 0.1), 1, randrange(0.001, 0.1))
	durmid = durlo * 5
	durhi = durlo * 10
	durti = 0.1

	amp = 400 * mast_amp * 0.02 / durlo
