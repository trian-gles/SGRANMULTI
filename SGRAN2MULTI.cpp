...
SGRAN2MULTI::SGRAN2MULTI() : branch(0)
{
	num_speakers = 0;
	prev_angle = -DBL_MAX;
	src_x = DBL_MAX;
...
int SGRAN2MULTI::init(double p[], int n_args)
{
/* NEW Args:
		p0: inskip
		p1: dur
	...
	*/
	...
	if (n_args < 21)
		return die("SGRAN2MULTI", "19 arguments are required");
	else if (n_args > 22)
		return die("SGRAN2MULTI", "too many arguments");

	if (SGRAN2MULTI_get_speakers(&num_speakers, speakers, &min_distance) == -1)
      return die("SGRAN2MULTI",
                 "Call SGRAN2MULTIspeakers before SGRAN2MULTI to set up speaker locations.");
...
double SGRAN2MULTI::prob(double low,double mid,double high,double tight)
...
void SGRAN2MULTI::setgains(Grain* grain)
{
//#ifdef DEBUG
//   rtcmix_advise("NPAN", "----------------------------------------------------------");
...
void SGRAN2MULTI::resetgrain(Grain* grain)
{
...
void SGRAN2MULTI::resetgraincounter()
{
...
int SGRAN2MULTI::calcgrainsrequired()
{
...
Instrument *makeSGRAN2MULTI()
{
	SGRAN2MULTI *inst = new SGRAN2MULTI();
	inst->set_bus_config("SGRAN2MULTI");
...
void rtprofile()
{
	RT_INTRO("SGRAN2MULTI", makeSGRAN2MULTI);
}
#endif
